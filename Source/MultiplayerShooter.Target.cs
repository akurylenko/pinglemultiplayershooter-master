...
public class MultiplayerShooterTarget : TargetRules
{
	...
	internal static void ApplySharedMultiplayerShooterTargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			MultiplayerShooterTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Log.TraceWarning("MultiplayerShooterGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}
